...
// --- KIỂM TRA QUYỀN ---
$isAdmin = isset($_SESSION['web_role']) && $_SESSION['web_role'] === 'admin';
$isLoggedIn = isset($_SESSION['web_user_id']);

// --- XỬ LÝ LOGOUT ---
if (isset($_GET['action']) && $_GET['action'] == 'logout') {
    unset($_SESSION['web_user_id'], $_SESSION['web_username'], $_SESSION['web_role']);
    ...
    <nav class="navbar navbar-expand-lg navbar-light navbar-custom sticky-top mb-4">
        <div class="container">
            ...
            <div class="d-flex align-items-center gap-2">
                <?php if ($isLoggedIn): ?>
                <span class="d-none d-sm-inline">Chào, <b><?= htmlspecialchars($_SESSION['web_username']) ?></b>
    ...
    <main class="container pb-5">
                ...
                <div
                    ...
                    <form method="GET" class="d-flex flex-column flex-sm-row gap-2 flex-grow-1 justify-content-end">
                        <div class="input-group input-group-sm flex-nowrap" style="min-width: 200px;">
                            ...
                            <input type="text" name="search" aria-label="Tìm kiếm địa điểm"
                                class="form-control border-start-0 ps-0" placeholder="Tìm tên quán, địa chỉ..."
                                value="<?= htmlspecialchars($search) ?>">
                        </div>
                        <div class="input-group input-group-sm flex-nowrap">
                            <span class="input-group-text bg-white border-end-0 text-secondary"><i
                                    class="bi bi-building"></i></span>
                            <select name="filter_city" id="filter_city" aria-label="Lọc theo thành phố"
                        ...
                        <div class="input-group input-group-sm flex-nowrap">
                            ...
                            <select name="filter_district" id="filter_district" aria-label="Lọc theo quận huyện"
                                class="form-select form-select-sm border-start-0 ps-0" style="min-width: 100px;"
                        ...
                        <div class="input-group input-group-sm flex-nowrap">
                            ...
                            <select name="filter_category" aria-label="Lọc theo danh mục"
                                class="form-select form-select-sm border-start-0 ps-0" onchange="this.form.submit()"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form method="POST" class="d-flex gap-2 mb-4 pb-4 border-bottom"><input type="hidden"
                                name="action" value="add_category"><input type="text" name="category_name"
                                class="form-control" required placeholder="VD: Trà sữa..."><button type="submit"
                        ...
                        <div style="max-height: 300px; overflow-y: auto;" class="pe-2"><?php foreach ($cats as $cat): ?>
                            <div class="d-flex gap-2 align-items-center mb-2 cat-row p-2 border rounded-3 bg-light">
                                <form method="POST" class="d-flex gap-2 flex-grow-1"><input type="hidden" name="action"
                                        ...
                                        class="btn btn-sm btn-success px-3"><i class="bi bi-check-lg"></i></button>
                                </form>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mt-3 text-muted small fst-italic">* Lưu ý: Sau khi thêm, trang sẽ tải lại để cập
                            nhật dữ
                            liệu.</div>
                    </div>
                </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center overflow-hidden">
                        <div id="wheel-container"
                            style="position: relative; width: 300px; height: 300px; margin: 0 auto;">
        ...
        function drawWheel() {
            ...
            currentPlaces.forEach((place, i) => {
                ...
                ctx.save();
                ctx.fillStyle = "white";
                ctx.translate(centerX + Math.cos(angle + arc / 2) * radius * 0.6, centerY + Math.sin(angle +
                    arc /
                    2) * radius * 0.6);
                ...
                cards.forEach(card => {
                    ...
                    if (!isNaN(placeLat) && !isNaN(placeLng)) {
                        ...
                        if (mapBtn) {
                            ...
                            mapBtn.target =
                                "_blank"; // Mở tab mới hoặc mở app Google Maps trên điện thoại
                        }
                    } else if (badge) {
